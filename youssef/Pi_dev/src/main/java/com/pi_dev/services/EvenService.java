...
public class EvenService implements IService<evenement> {
    ...
    public void ajouter(evenement evenement) {
        String req = "INSERT INTO evenement (titre, description, date, statut, categorie, image, prix) VALUES (?, ?, ?, ?, ?, ?, ?)";
        try {
            ...
            pst.setDouble(7, evenement.getPrix()); // Ajout du prix
            pst.executeUpdate();
    ...
    public void modifier(evenement evenement) {
        String req = "UPDATE evenement SET titre = ?, description = ?, date = ?, statut = ?, categorie = ?, image = ?, prix = ? WHERE id = ?";
        try {
            PreparedStatement pst = connection.prepareStatement(req);
            pst.setString(1, evenement.getTitre());
            pst.setString(2, evenement.getDescription());
        ...
        try {
            ...
            while (rs.next()) {
                evenement evt = new evenement(
                        ...
                        rs.getString("image"),
                        rs.getDouble("prix") // Récupération du prix
                );
                evenements.add(evt);
            }
        } catch (SQLException e) {
